...
const dotnetUrl = new URL(
	`${import.meta.env.BASE_URL}assets/dotnet/dotnet.js`,
...
export const makeDotnetCompiler: CompilerFactory<Streams, Program> = async (ctx, streams) => {
	...
	const compiler = await inContext(
		ctx,
		new DotnetCompilerFactory(log, libsLoader, compilerModule).create(LIBS)
	);
