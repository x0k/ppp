<script lang="ts" generics="T extends string">
	...
	let { value = $bindable(), options, label, preLabel, postLabel, children }: Props = $props();
...
<details bind:this={detailsElement} class="dropdown dropdown-end dropdown-top">
	<summary class="btn min-w-24 btn-ghost font-normal btn-sm">
		{#if label}
			{@render label(value)}
		{:else}
			{value}
	...
	<div
		tabindex="0"
		class="dropdown-content max-h-[calc(100vh-10rem)] overflow-y-auto rounded-box border border-white/5 bg-base-200 text-base-content shadow-2xl outline-1 outline-black/5"
	>
