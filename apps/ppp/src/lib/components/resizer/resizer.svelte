...
<div
	...
	class={[
		'absolute z-50 select-none hover:bg-primary/50 active:bg-primary',
		alignment === Alignment.Start && 'top-0 left-0',
		alignment === Alignment.End && 'right-0 bottom-0',
		orientation === Orientation.Horizontal && 'h-1 w-full hover:cursor-row-resize',
		orientation === Orientation.Vertical && 'h-full w-1 hover:cursor-col-resize'
	]}
></div>
