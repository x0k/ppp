...
export function try_fold(loop$collection, loop$accumulator, loop$fun) {
  while (true) {
    ...
    let accumulator = loop$accumulator;
    let fun = loop$fun;
    if (collection instanceof $Empty) {
    ...
    if (list instanceof $Empty) {
      ...
      if (direction instanceof Ascending) {
        if ($ instanceof $order.Lt) {
          ...
          let _block;
          if (direction instanceof Ascending) {
            _block = listPrepend(do_reverse(growing$1, toList([])), acc);
          } else {
...
export function transpose(loop$list_of_list) {
  while (true) {
    ...
    if (list_of_list instanceof $Empty) {
      return list_of_list;
    } else {
      let $ = list_of_list.head;
      if ($ instanceof $Empty) {
