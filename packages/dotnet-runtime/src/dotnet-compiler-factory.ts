...
export class DotnetCompilerFactory {
	constructor(
		...
		protected readonly compilerModule: DotnetModule<CompilerModuleImports, CompilerModuleExports>
	) {}

	async create(dllUrls: string[]): Promise<DotnetCompiler> {
		const { setModuleImports, getAssemblyExports, getConfig } = this.compilerModule;
