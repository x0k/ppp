...
export function testCase<I, O>(test: (input: I) => O): (input: I) => TestCase<I, O> {
	return (input) => ({
		input,
		output: test(input)
	});
...
export type TestCompilerFactory<Options, I, O> = CompilerFactory<Options, TestProgram<I, O>>;
	...
	for (; i < testsCases.length; i++) {
		...
		try {
			...
			if (!isDeepEqual(result, testCase.output)) {
				log.error(`Test case failed, expected "${testCase.output}", but got "${result}"`);
				return i;
			}
		} catch (err) {
