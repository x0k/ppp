...
export type UniversalFactory<D, I, O> = (ctx: Context, data: D) => Promise<TestCompiler<I, O>>;
	...
	constructor(
		...
		const handlers: Handlers<I, O> = {
			initialize: async ({ universalFactoryFunction, buffer }) => {
				const universalFactory =
					await evalEntity<UniversalFactory<D, I, O>>(universalFactoryFunction);
				this.compiler = await superFactory(
					this.compilerCtx.ref,
					createStreamsClient(new SharedQueue(buffer), {
						write(data) {
							connection.send({
...
export function makeRemoteTestCompilerFactory<D, I, O>(
	...
	return async (
		...
		const connection = new WorkerConnection<Outgoing<I, O>, Incoming<I, O>>(worker);
		connection.start(ctx);
		...
		const remote = startRemote<Handlers<I, O>, string, TestingActorEvent>(ctx, log, connection, {
			write(data) {
				output.write(data);
			},
			error(err) {
