	...
	constructor(
		...
		const handlers: Handlers = {
			initialize: async (buffer: SharedArrayBuffer) => {
				this.compiler = await compilerFactory(
...
export function startCompilerActor(
	...
	const connection = new WorkerConnection<Incoming, Outgoing>(self as unknown as Worker);
...
export function makeRemoteCompilerFactory(Worker: WorkerConstructor) {
	return async (
		...
		const remote = startRemote<Handlers, string, CompilerActorEvent>(ctx, log, connection, {
			write(data) {
				output.write(data);
			},
			error(err) {
