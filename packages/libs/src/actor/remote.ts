...
export function startRemote<H extends Handlers, E, Event extends EventMessage<string, any>>(
	ctx: Context,
	log: Logger,
	...
	let lastId = 0;
	const promises = new Map<RequestId, DeferredPromise<ReturnType<H[keyof H]>, E | CanceledError>>();
	ctx.onCancel(() => {
		...
		{},
		{
			get(_, prop) {
				...
				return (arg: Parameters<H[typeof request]>[0]) => {
					...
					const promise = new Promise<ReturnType<H[typeof request]>>((resolve, reject) => {
