...
export interface EventMessage<T extends PropertyKey, P> extends AbstractMessage<MessageType.Event> {
	event: T;
	payload: P;
}
...
export interface ResponseMessage<R> extends AbstractMessage<MessageType.Response> {
	id: RequestId;
	result: R;
}
...
export type IncomingMessage<H extends Handlers> = {
	[K in keyof H]: RequestMessage<K, Parameters<H[K]>[0]> | EventMessage<K, Parameters<H[K]>[0]>;
}[keyof H];
