...
export class JVM extends Doppio.VM.JVM {
	...
	override _initSystemProperties(
		...
		super._initSystemProperties(bootstrapClasspath, javaClassPath, javaHomePath, tmpDir, opts);
		...
		bootupTasks.push((next: (err?: any) => void) => {
			...
			clCons['java/lang/ClassLoader/getSystemClassLoader()Ljava/lang/ClassLoader;'](
				firstThread,
				//@ts-expect-error implementation error
				null,
				(e?: JVMTypes.java_lang_Throwable, rv?: JVMTypes.java_lang_ClassLoader) => {
...
export function createJVM(opts: Doppio.VM.Interfaces.JVMOptions): Promise<Doppio.VM.JVM> {
	return new Promise(
		(resolve, reject) => new Doppio.VM.JVM(opts, (e, jvm) => (e ? reject(e) : resolve(jvm!)))
	);
}

const noop = () => {};

export function makeJVMFactory(bootstrapClassLoaderFactory: BootstrapClassLoaderFactory) {
