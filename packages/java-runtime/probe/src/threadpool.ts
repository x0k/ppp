...
export interface Thread {
  getStatus(): VM.Enums.ThreadStatus;
  isDaemon(): boolean;
  getPriority(): number;
  setStatus(status: VM.Enums.ThreadStatus): void;
  run(): void;
}
 ...
 */
function isRunnable(status: VM.Enums.ThreadStatus): boolean {
  return status === VM.Enums.ThreadStatus.RUNNABLE;
}
...
class WeightedRoundRobinScheduler<T extends Thread> implements Scheduler<T> {
  ...
  private runThread(): void {
    ...
    setImmediate(() => {
      ...
      if (queue.length > 0) {
        ...
        assert(
          thread.getStatus() === VM.Enums.ThreadStatus.RUNNABLE,
  ...
  public unscheduleThread(thread: T): void {
    ...
    assert(
      queue.indexOf(thread) > -1,
      `Tried to unschedule thread that was not scheduled.`
    );
  ...
  public quantumOver(thread: T): void {
    assert(
      this._queue[0] === thread,
      `A non-running thread has an expired quantum?`
    );
    this._count++;
    if (
 ...
 */
export class ThreadPool<T extends Thread> {
  private threads: T[] = [];
   ...
   */
  private emptyCallback: (() => boolean) | null;
  ...
  private anyNonDaemonicThreads(): boolean {
    for (let i = 0; i < this.threads.length; i++) {
      ...
      if (
        status !== VM.Enums.ThreadStatus.NEW &&
        status !== VM.Enums.ThreadStatus.TERMINATED
      ) {
        return true;
      }
    }
  ...
  public statusChange(
    thread: T,
    oldStatus: VM.Enums.ThreadStatus,
    newStatus: VM.Enums.ThreadStatus
  ): void {
    var wasRunnable = isRunnable(oldStatus),
      nowRunnable = isRunnable(newStatus);

    if (
      oldStatus === VM.Enums.ThreadStatus.NEW ||
      oldStatus === VM.Enums.ThreadStatus.TERMINATED
    ) {
    ...
    if (newStatus === VM.Enums.ThreadStatus.TERMINATED) {
